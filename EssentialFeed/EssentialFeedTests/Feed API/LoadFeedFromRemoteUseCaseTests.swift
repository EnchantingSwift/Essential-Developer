...
class LoadFeedFromRemoteUseCaseTests: XCTestCase {
    ...
    func test_load_deliversErrorOnNon200HTTPResponse() {
        ...
        samples.enumerated().forEach { index, code in
            expect(sut, toCompleteWith: failure(.invalidData), when: {
                let emptyJSONList = makeItemsJSON(with: [])
    ...
    func test_load_deliversItemsOn200HTTPResponseWithJSONItems() {
        ...
        expect(sut, toCompleteWith: .success([item1.model, item2.model]), when: {
            client.complete(withStatusCode: 200, data: makeItemsJSON(with: [item1.json, item2.json]))
        })
    }
    
    func test_load_doesNotDeliverResultAfterInstanceSUTHasBeenDeallocated() {
    ...
    private func makeItem(description: String? = nil, location: String? = nil, imageURL: URL = URL(string: "http://any-image-url.com")!) -> (model: FeedImage, json: [String: Any]) {
    ...
    private func makeItemsJSON(with jsonItems: [[String: Any]]) -> Data {
        return try! JSONSerialization.data(withJSONObject: ["items": jsonItems])
    }
    ...
    private func expect(_ sut: RemoteFeedLoader, toCompleteWith expectedResult: RemoteFeedLoader.Result, when action: () -> Void, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.load { receivedResult in
            ...
            case let (.success(expectedItem), .success(receivedItem)):
                XCTAssertEqual(expectedItem, receivedItem, file: file, line: line)
